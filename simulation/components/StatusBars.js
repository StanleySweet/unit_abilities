...
StatusBars.prototype.AddInfiltrationBar = function(cmpOverlayRenderer, yoffset)
{
	if (!this.enabled)
		return 0;

	if (typeof IID_Infiltrator == "undefined")
	{
		if (typeof IID_InfiltrationManager == "undefined")
			return 0;

		const cmpInfiltrationManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_InfiltrationManager);
		const progress = cmpInfiltrationManager ? cmpInfiltrationManager.GetTargetProgress(this.entity) : 0;
		return progress > 0 ? this.AddBar(cmpOverlayRenderer, yoffset, "upgrade", progress) : 0;
	}

	const cmpInfiltrator = Engine.QueryInterface(this.entity, IID_Infiltrator);
	if (cmpInfiltrator && cmpInfiltrator.IsInfiltrating())
		return this.AddBar(cmpOverlayRenderer, yoffset, "upgrade", cmpInfiltrator.GetProgress());
